...
import { api, type InspectionActivity, type MobileApp } from "@/lib/api";
...
export default function MobileActivityPage() {
  ...
  const load = async () => {
    ...
    try {
      const [statusResponse, activityResponse] = await Promise.all([
        api.mobile.status(session.access_token),
        api.mobile.activity(session.access_token, 60),
      ]);
  ...
  useEffect(() => {
    ...
    return subscribeToEvents(
      (event) => {
        if (
          event.type === "mobile-inspection.activity" ||
          event.type === "mobile-inspection.status"
        ) {
          void load();
        }
      },
      {
        types: ["mobile-inspection.activity", "mobile-inspection.status"],
      }
    );
    ...
    <div className="space-y-8">
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <SectionHeading icon={Activity} title="Mobile Activity" tone={headingTone} />
          <Button
            type="button"
            variant="outline"
            onClick={() => void load()}
        ...
        <p className="max-w-3xl text-sm leading-6 text-slate-500 dark:text-slate-400">
          A single system-wide timeline for mobile inspections, AI analysis, and follow-up actions. This keeps trust high because users can see exactly what the agent did and when.
        </p>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-transparent">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            ...
            <h3 className="mt-2 text-xl font-semibold text-slate-900 dark:text-white">
              Inspections, syncs, and AI actions
            </h3>
          </div>
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-8 text-sm text-slate-500 dark:border-slate-800 dark:bg-slate-950/30 dark:text-slate-400">
            No mobile app is connected yet. Once you upload an APK or IPA, this page will show only real mobile-device inspection activity.
          </div>
        ) : activity.length > 0 ? (
          <div className="space-y-3">
            {activity.map((item) => (
              <div
                key={item.id}
                className={cn("rounded-xl border px-4 py-4", rowTone(item.status))}
              >
          ...
          <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-8 text-sm text-slate-500 dark:border-slate-800 dark:bg-slate-950/30 dark:text-slate-400">
            No activity yet. Once a mobile inspection runs, the system timeline will start filling in automatically.
          </div>
        )}
      </section>
    </div>
