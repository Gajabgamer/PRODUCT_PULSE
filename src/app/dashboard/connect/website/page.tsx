...
export default function WebsitePage() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!session?.access_token) {
      setError("Please sign in before updating the website source.");
      return;
    }
    ...
    try {
      ...
      await refreshSetup();
      setMessage("Website source updated");
    } catch (err) {
  ...
  return (
    <div className="space-y-8">
      <section className="space-y-4">
        <SectionHeading icon={Globe2} title="Website Source" tone={headingTone} />
        <p className="max-w-3xl text-sm leading-6 text-slate-500 dark:text-slate-400">
          Configure the website AgenticPulse should inspect. This is the source used by the web inspection agent for health checks, issue reproduction, and authenticated product flows.
        </p>
      </section>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Optional test credentials for authenticated web inspection.
              </p>
            </div>
          </div>
        ...
        <div className="flex justify-end">
          ...
          >
            ...
            {saving ? "Saving..." : "Save Website Source"}
          </Button>
        </div>
      </form>
    </div>
